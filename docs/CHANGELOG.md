...
- 新增 `UI_ICON_RESOURCES.md`，记录当前 104 个随包 PNG 图标的完整分类、97 个已使用与 7 个预留状态，以及后续新增、替换和运行时一致性验收流程。
- 文件工作区的列表/网格视图切换与应用内标题栏的通知、设备中心、设置入口接入新增 PNG 图标；标题栏品牌标识改为复用应用窗口和任务栏的 `app.png`。
- 文件工作区的主页、导航、刷新、文件操作与详情入口，显示工作区的信息卡与浅色/深色模式，镜像来源选择以及应用窗口和任务栏，接入新增的 20 个 PNG 图标。
- 侧边栏及对应主工作区接入 `resources/images/icons/` 下的 PNG 图标；新增统一运行时图片加载接口，并在构建和安装阶段复制到 `runtime/images/icons/`。
